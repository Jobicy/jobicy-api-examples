...
import { sendDiscordJob } from "./discord.js";
...
if (!webhookUrl) {
  console.error("Set DISCORD_WEBHOOK_URL in .env before starting the integration.");
  process.exit(1);
}
...
async function checkJobs() {
  ...
  for (const job of unseen) {
    await sendDiscordJob(webhookUrl, job);
...
await store.load();
console.log(`Jobicy Discord integration started; checking every ${intervalSeconds} seconds.`);
await run();
